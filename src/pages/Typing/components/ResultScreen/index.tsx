...
import IconXiaoHongShu from '~icons/simple-icons/xiaohongshu'
          ...
          <div className="my-card fixed flex w-[90vw] max-w-6xl flex-col overflow-hidden rounded-3xl bg-white pb-14 pl-10 pr-5 pt-10 shadow-lg dark:bg-gray-800 md:w-4/5 lg:w-3/5">
            <div className="text-center font-sans text-xl font-normal text-gray-900 dark:text-gray-400 md:text-2xl">
              {`${currentDictInfo.name} ${
                isReviewMode ? (state.strengthenSession.enabled ? '需要加强' : '错题复习') : '第' + (currentChapter + 1) + '章'
              }`}
            </div>
            ...
            <div className="mt-10 flex flex-row gap-2 overflow-hidden">
              ...
              <div className="ml-2 flex flex-col items-center justify-end gap-3 text-xl">
                <a
                  href="https://www.mycloudai.org/"
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-md border border-gray-300 px-2 py-1 text-xs font-semibold text-gray-600 transition-colors hover:border-orange-300 hover:text-orange-600 dark:border-gray-600 dark:text-gray-300 dark:hover:border-orange-500 dark:hover:text-orange-400"
                  title="访问 MyCloudAI 首页"
                >
                  MyCloudAI
                </a>
                {!isReviewMode && (
