...
export default function WordComponent({ word, onFinish }: { word: Word; onFinish: () => void }) {
  ...
  // Immediately record to the error bank as soon as the first wrong character is typed.
  // This makes the word appear in the sidebar error list without waiting for the word to finish.
  const immediateWrongSavedRef = useRef(false)
  useEffect(() => {
    // Reset when the word itself changes (new word presented)
    immediateWrongSavedRef.current = false
  }, [word.name])
  useEffect(() => {
    if (wordState.wrongCount === 1 && !immediateWrongSavedRef.current) {
      immediateWrongSavedRef.current = true
      const record = new WordRecord(word.name, dictID, isRevision ? -1 : currentChapter, [], 1, {})
      db.wordRecords
        .add(record)
        .then(() => bumpStrengthenRefresh((t) => t + 1))
        .catch(() => {
          /* ignore write errors */
        })
    }
  ...
  useEffect(() => {
    if (wordState.inputWord.length !== 0 || !state.isTyping || !pronunciationIsOpen) return
    ...
    const timer = window.setTimeout(tryPlay, 50)
    return () => {
      cancelled = true
      window.clearTimeout(timer)
    }
  }, [state.isTyping, wordState.inputWord.length, word.name, pronunciationIsOpen])
