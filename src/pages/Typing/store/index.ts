...
export enum TypingStateActionType {
  ...
  ADVANCE_STRENGTHEN_WORD = 'ADVANCE_STRENGTHEN_WORD',
}
...
export type TypingStateAction =
  | {
      ...
      payload: {
        ...
        initialIndex?: number
        strengthenConfig?: { enabled: boolean; targetCorrectCount: number }
      }
    }
  ...
  | { type: TypingStateActionType.ADVANCE_STRENGTHEN_WORD }
...
export const typingReducer = (state: TypingState, action: TypingStateAction) => {
  switch (action.type) {
    case TypingStateActionType.SETUP_CHAPTER: {
      ...
      if (action.payload.strengthenConfig?.enabled) {
        newState.strengthenSession.enabled = true
        newState.strengthenSession.targetCorrectCount = action.payload.strengthenConfig.targetCorrectCount
      }
    ...
    case TypingStateActionType.ADVANCE_STRENGTHEN_WORD: {
      const currentIndex = state.chapterData.index
      const currentWord = state.chapterData.words[currentIndex]
      const currentLog = state.chapterData.userInputLogs[currentIndex]
      if (!currentWord || !currentLog) break

      const wordKey = currentWord.name
      const prevCorrect = state.strengthenSession.correctByWord[wordKey] ?? 0
      // Reset to 0 on ANY wrong attempt — only consecutive clean runs count
      const nextCorrect = currentLog.wrongCount === 0 ? prevCorrect + 1 : 0
      state.strengthenSession.correctByWord[wordKey] = nextCorrect

      // Consume this attempt and move forward in strengthen loop.
      state.chapterData.wordCount += 1
      state.isShowSkip = false

      const shouldRemove = nextCorrect >= state.strengthenSession.targetCorrectCount
      state.chapterData.words.splice(currentIndex, 1)
      state.chapterData.userInputLogs.splice(currentIndex, 1)

      if (!shouldRemove) {
        state.chapterData.words.push(currentWord)
        state.chapterData.userInputLogs.push({ ...structuredClone(initialUserInputLog), index: state.chapterData.userInputLogs.length })
      }

      state.chapterData.userInputLogs = state.chapterData.userInputLogs.map((log, index) => ({ ...log, index }))
      ...
      if (currentIndex >= state.chapterData.words.length) {
